...
date
TENANT=$1
SERVER="dba-postgres-dev-42.ist.berkeley.edu port=5113 sslmode=prefer"
...
# 
##############################################################################
time psql -F $'\t' -R"@@" -A -U $USERNAME -d "$CONNECTSTRING"  -f ~/bin/bampfa_website_extract.sql -o bwe.tab
